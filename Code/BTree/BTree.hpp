...
template <typename T>
BTNode<T>* BTree<T>::search ( const T& e ) { //在B-树中查找关键码e
...
bool BTree<T>::remove ( const T& e ) { //从BTree树中删除关键码e
    BTNode<T>* v = search ( e ); if ( !v ) return false; //确认目标关键码存在
    int r = v->key.search ( e ); //确定目标关键码在节点v中的秩（由上，肯定合法）
...
template <typename T>
void BTree<T>::release ( BTNode<T>* v ) {
    if(!v)return;
    for(int i = 0;i < v->child.size();i++)release(v->child[i]);
    delete v;
}
