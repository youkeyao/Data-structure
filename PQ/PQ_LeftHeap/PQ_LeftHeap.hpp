...
class PQ_LeftHeap : public PQ<T>, public BinTree<T> { //基于二叉树，以左式堆形式实现的PQ
    public:
        ...
        //静态方法
    static BinNode<T>* merge ( BinNode<T>* a, BinNode<T>* b ) { //根据相对优先级确定适宜的方式，合并以a和b为根节点的两个左式堆
        if ( ! a ) return b; //退化情况
        if ( ! b ) return a; //退化情况
        if ( a->data < b->data ) _swap ( a, b ); //一般情况：首先确保b不大
        ( a->rc = merge ( a->rc, b ) )->parent = a; //将a的右子堆，与b合并
        if ( !a->lc || a->lc->npl < a->rc->npl ) //若有必要
        _swap ( a->lc, a->rc ); //交换a的左、右子堆，以确保右子堆的npl不大
